...
import {
	createOrgProfileAPI,
	CreateOrgProfileType,
} from "@/services/api/organization/create_org_profile";

const createOrgProfileSchema = z.object({
...
export default function CreateOrgProfileDialog(props: Props) {
	const form = useForm<z.infer<typeof createOrgProfileSchema>>({
		resolver: zodResolver(createOrgProfileSchema),
	});

	//mutation
	const mutation = useMutation({
		mutationKey: ["createOrgProfile"],
		mutationFn: (data: CreateOrgProfileType) => createOrgProfileAPI(data),
		onSuccess: () => {
			// Invalidate org profile get request
		},
	});

	const onSubmit = (data: z.infer<typeof createOrgProfileSchema>) => {
